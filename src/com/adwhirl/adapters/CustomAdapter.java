...
public class CustomAdapter extends AdWhirlAdapter {
  ...
  public void displayCustom() {
    ...
    switch (adWhirlLayout.custom.type) {
      case AdWhirlUtil.CUSTOM_TYPE_BANNER:
        ...
        Bitmap bm = ((BitmapDrawable)adWhirlLayout.custom.image).getBitmap(); 
        
        bm = resizeBitmap(bm, AdWhirlUtil.getDensity(activity));
  ...
  private Bitmap resizeBitmap(Bitmap bm, double density){
	  
	if (1 >= density) {
  		bm = Bitmap.createScaledBitmap(bm, 320, 50, true);
  	}else if(1.5 == density) {
  		bm = Bitmap.createScaledBitmap(bm, 480, 75, true);
  	} else if(2 <= density) {
  		bm = Bitmap.createScaledBitmap(bm, 640, 100, true);
  	}
	return bm;
  }
